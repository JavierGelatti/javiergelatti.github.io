...
`,S=`<h1>Automated refactorings for Godot</h1>
...
`,F=new Map([["/chorded-keyset.md",T],["/diagramas-colaboracion.md",A],["/game-of-life.md",I],["/godot-refactorings.md",S],["/index.md",L],["/lambda-calculus-editor.md",C],["/logica-combinatoria.md",P],["/mini-alto.md",E],["/ruby-live-typing.md",z],["/ruby-refactorings.md",R],["/self-js.md",_]]),m=document.querySelector("main"),c=m.querySelector("article"),v=m.querySelector("svg"),x=M(g());x!==void 0&&(c.innerHTML=x);const l=[];Promise.all([...c.querySelectorAll("a")].map(k)).then(W);function B(a){let s=0;for(const e of[...a]){const i=e.charCodeAt(0);s=(s<<5)-s+i,s|=0}return s}async function k(a){const s=new URL(a.href),e=g();if(Y(s)){if(s.origin===e.origin&&s.pathname===e.pathname&&s.hash!==""){a.classList.add("hash");return}}else{a.target="_blank",a.rel="noopener noreferrer",a.classList.add("external");return}const i=Math.abs(B(a.href))%360;a.style.background=`hsl(${i}, 100%, 40%)`,a.classList.add("local")}c.addEventListener("scroll",()=>p());c.parentElement.addEventListener("scroll",()=>p());window.addEventListener("resize",()=>p());const D=new ResizeObserver(()=>p());function p(){l.forEach(a=>{const s=a.svgPath,e=a.sourceParentElement.getBoundingClientRect(),i=a.sourceLink.getBoundingClientRect(),t=document.body.scrollLeft,n=document.body.scrollTop;let o=i.x-1+i.width+t,r=i.y+i.height/2+n;const f=a.element.getBoundingClientRect(),h=f.x+t,d=f.y+10+n;if(r<e.y)o=e.right+t,r=e.top+8+n;else if(r>e.bottom){const b=e.bottom;o+=(b-r)*(h-o)/(d-r),r=b}s.classList.toggle("out",r===e.bottom);const y=20;s.setAttribute("d",`
            M ${o},${r}
            C ${o+y},${r} ${h-y},${d} ${h},${d}
        `)})}function g(){return new URL(document.location.href)}function M(a){if(a.origin===g().origin)return F.get(a.pathname)}function O(a){return a.closest(".content-viewer")??void 0}function j(){sessionStorage.setItem("sideViewers",JSON.stringify(l.map(a=>{const s=a.sourceLink.href,e=q(a.sourceParentElement,s).indexOf(a.sourceLink);return{href:s,linkIndex:e}})))}function q(a,s){return[...a.querySelectorAll("a")].filter(e=>new URL(e.href).toString()===s)}function W(){const a=sessionStorage.getItem("sideViewers")??"[]";JSON.parse(a).forEach(({href:e,linkIndex:i})=>{var r;const t=((r=l[l.length-1])==null?void 0:r.element)??c,n=q(t,e);if(n.length===0)throw new Error("Link not found");(n.length>1?n[i]:n[0]).click()})}function w(a,s){const e=O(a),i=l.find(o=>o.sourceParentElement===e);i&&u(i);const t=document.createElement("article");t.className="content-viewer",t.replaceChildren(s),D.observe(t);const n=document.createElementNS("http://www.w3.org/2000/svg","path");n.style.stroke=a.style.background,v.append(n),N({element:t,sourceLink:a,sourceParentElement:e,svgPath:n})}function H(a){const s=new URL(a.href),e=M(s);e===void 0?w(a,J(s)):w(a,G(e))}function J(a){const s=document.createElement("iframe");return s.src=a.toString(),s.classList.add("loading"),s.onload=()=>s.classList.remove("loading"),s}function G(a){const s=document.createElement("div");return s.style.display="contents",s.innerHTML=a,s.querySelectorAll("a").forEach(k),s.querySelectorAll("iframe").forEach(e=>{e.addEventListener("load",()=>{var t;const i=e.contentDocument||((t=e.contentWindow)==null?void 0:t.document);i!==void 0&&new ResizeObserver(([n])=>{e.style.height=`${n.target.scrollHeight}px`}).observe(i.documentElement)})}),s}function N(a){l.push(a),m.style.setProperty("--number-of-columns",String(l.length+1)),m.insertBefore(a.element,v),p(),a.element.addEventListener("scroll",()=>p()),a.element.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"}),j()}function u(a){a.svgPath.remove(),a.element.remove(),a.sourceLink.classList.remove("active");const s=l.indexOf(a),e=l.at(s+1);e&&u(e),l.splice(s,1),m.style.setProperty("--number-of-columns",String(l.length+1)),j()}function Y(a){return a.hostname==="javiergelatti.github.io"||a.hostname==="localhost"||/^\d+\.\d+\.\d+\.\d+$/.test(a.hostname)}document.documentElement.addEventListener("click",a=>{var e;if(!(a.target instanceof HTMLElement))return;const s=(e=a.target)==null?void 0:e.closest("a");if(s&&s.classList.contains("local"))if(a.preventDefault(),s.classList.contains("active")){const i=l.find(t=>t.sourceLink===s);u(i)}else s.classList.add("active"),H(s)});
